...
# Valores de k y n
k_values = [5, 15, 25, 50, 75]
n_values = [1000, 1150, 1300, 1450, 1600, 1750, 1900, 2050, 2200, 2400]
...
df_bruteforce = pd.DataFrame(bruteforce_times, columns=n_values, index=k_values)
df_kd_tree = pd.DataFrame(kd_tree_times, columns=n_values, index=k_values)

# Graficar los resultados
for k in k_values:
    plt.figure(figsize=(8, 6))
    plt.plot(df_bruteforce.loc[k], label="Bruteforce", marker='o')
    plt.plot(df_kd_tree.loc[k], label="KD-Tree", marker='o')
    plt.title(f"Cluster Points, k={k}")
